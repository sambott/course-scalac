...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    val stringCol = columnNames.head
    val numericCols = columnNames.tail
    StructType(StructField(stringCol, StringType, nullable = false) :: numericCols.map(s => StructField(s, DoubleType, nullable = false)))
  }
  ...
  /** @return An RDD Row compatible with the schema produced by `dfSchema`
    ...
    */
  def row(line: List[String]): Row = Row(line.head :: line.tail.map(d => d.toDouble) : _*)
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryPrefixes = List[String]("t01", "t03", "t11", "t1801", "t1803")
    val workingPrefixes = List[String]("t05", "t1805")
    val otherPrefixes = List[String]("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")
    @tailrec
    def inner(cols: List[String], primaries: List[Column], workings: List[Column], others: List[Column]): (List[Column], List[Column], List[Column]) = {
      cols match {
        case Nil => (primaries, workings, others)
        case head :: tail if primaryPrefixes.exists(p => head.startsWith(p)) =>
          inner(tail, primaries :+ col(head), workings, others)
        case head :: tail if workingPrefixes.exists(p => head.startsWith(p)) =>
          inner(tail, primaries, workings :+ col(head), others)
        case head :: tail if otherPrefixes.exists(p => head.startsWith(p)) =>
          inner(tail, primaries, workings, others :+ col(head))
        case _ => throw new UnsupportedOperationException("Unknown categroy")
      }
    }
    inner(columnNames, Nil, Nil, Nil)
  }
