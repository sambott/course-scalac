...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    ...
    downsweepSequential(input, output, 0, 0, len)
  }
  ...
  case class Leaf(from: Int, until: Int, maxPrevious: Float) extends Tree
    ...
    def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
      ...
      var maxAngle = 0f
      while (i < until) {
        maxAngle = max(maxAngle, calcAngle(input)(i))
        i += 1
      }
      maxAngle
    }
  ...
  def calcAngle(input: Array[Float])(i: Int): Float = if (i == 0) 0f else input(i)/i
