...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Empty else {
      val next = initial flatMap {
        case (b, h) => newNeighborsOnly(neighborsWithHistory(b, h), explored)
      }
      def nextExplored = explored ++ next.map(_._1)
      val nextStream = from(next, nextExplored)
      initial ++ nextStream
  }}
  ...
  lazy val solution: List[Move] = {
    pathsToGoal.toList.headOption.map(_._2.reverse).getOrElse(Nil)
  }
}
